  ...
  try {
    ...
    if (userResult.length === 0 || !userResult[0].user_id) {
      ...
      return res.status(200).json({
        message: "Order updated, but user notification skipped",
        riders: [],
        notifications: []
      });
    }
    ...
    switch (order_status) {
      case 1: // Vendor confirmed order
        ...
        // Get nearby riders and send them notifications
        if (vendor_id) {
    ...
    // Step 5: Wait for all notifications to settle
    const notifResults = await Promise.allSettled(notifications);

    const notificationStatuses = notifResults.map((r, i) => ({
      userId: notifications[i]?.userId ?? null,
      status: r.status,
